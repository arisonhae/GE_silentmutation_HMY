...
> **⚠️ window에 인트론이 섞여 있으면 exon-aware 체크박스를 켜고 exon 구간을 마킹할 것
> (꺼짐이 기본값).** 끄면 툴이 window 전체를 CDS로 가정해 인트론을 코돈으로 읽고,
> 인트론 염기에 "silent" 치환을 제안하게 된다. silent 마커를 찾는 window는 변이 기준
> ±60bp라, 변이가 exon 가장자리 근처면 인트론이 딸려 들어온다.
---
