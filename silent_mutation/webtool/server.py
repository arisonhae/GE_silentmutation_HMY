...
# NOTE: deepprime_runner (genet + pandas) is imported lazily inside the /api/analyze
# use_dp branch, so the standalone server boots with only Flask installed.
...
def analyze():
    ...
    if d.get("use_dp"):
        ...
        try:
            ...
            return jsonify({**resp, "deepprime": {
                "available": False, "pegrnas": [],
                "message": ("DeepPrime ranking needs the genet stack (pandas + "
                            "genet). Install requirements-deepprime.txt in the "
                            "'genet' conda env, or use the standalone candidate "
                            "list. Import error: %s" % e),
            }})
        try:
