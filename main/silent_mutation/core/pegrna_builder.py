...
def build_pegrna_output(
    ...
    return PegRNAOutput(
        ...
        variant_near_exon_boundary=variant.near_exon_boundary,
        variant_nearest_boundary_dist=variant.nearest_boundary_dist,
...
# ─────────────────────────────────────────────────────────────────────────────
# Multi-variant DataFrame
# ─────────────────────────────────────────────────────────────────────────────

_DEEPPRIME_CORE_COLS = ["ID", "target", "wtseq", "edseq"]
_PEGRNA_OUTPUT_COLS = _DEEPPRIME_CORE_COLS + [
    "gene_symbol", "transcript_id", "chrom", "genome_pos",
    "ref_allele", "alt_allele", "cds_strand",
    "variant_type", "ref_len", "alt_len",
    # variant protein change
    "variant_protein_change", "variant_wt_codon", "variant_mut_codon",
    "variant_wt_aa", "variant_mut_aa", "variant_codon_index",
    # splice context (variant-level + RTT-level)
    "variant_near_exon_boundary", "variant_nearest_boundary_dist",
    "cds_bases_in_rtt", "rtt_covers_intron", "splice_junction_codons_in_rtt",
    # co-edited window
    "seq_ed_with_silent",
    # pam
    "pam_pos", "pam_pattern", "nick_pos",
    "pbs_seq", "rtt_seq_wt",
    "rtt_start_in_seq", "rtt_end_in_seq",
    # silent
    "silent_positions", "silent_ref", "silent_alt",
    "original_codon", "mutated_codon", "aa",
    "locale", "priority",
]


def _row_to_dict(row: PegRNAOutput, target_tag: str = "") -> dict:
    return {
        "ID": row.variant_id,
        "target": target_tag or row.gene_symbol,
        "wtseq": row.seq_wt,
        "edseq": row.seq_ed,
        "gene_symbol": row.gene_symbol,
        "transcript_id": row.transcript_id,
        "chrom": row.chrom,
        "genome_pos": row.genome_pos,
        ...
        "ref_len": row.ref_len,
        "alt_len": row.alt_len,
        "variant_protein_change": row.variant_protein_change,
        "variant_wt_codon": row.variant_wt_codon,
        "variant_mut_codon": row.variant_mut_codon,
        "variant_wt_aa": row.variant_wt_aa,
        "variant_mut_aa": row.variant_mut_aa,
        "variant_codon_index": row.variant_codon_index,
        "variant_near_exon_boundary": row.variant_near_exon_boundary,
        "variant_nearest_boundary_dist": row.variant_nearest_boundary_dist,
        "cds_bases_in_rtt": row.cds_bases_in_rtt,
        "rtt_covers_intron": row.rtt_covers_intron,
        "splice_junction_codons_in_rtt": row.splice_junction_codons_in_rtt,
        "seq_ed_with_silent": row.seq_ed_with_silent,
        "pam_pos": row.pam_pos,
        "pam_pattern": row.pam_pattern,
        "nick_pos": row.nick_pos,
        "pbs_seq": row.pbs_seq,
        "rtt_seq_wt": row.rtt_seq_wt,
        "rtt_start_in_seq": row.rtt_start_in_seq,
        "rtt_end_in_seq": row.rtt_end_in_seq,
        "silent_positions": row.silent_positions,
        "silent_ref": row.silent_ref,
        "silent_alt": row.silent_alt,
        "original_codon": row.original_codon,
        "mutated_codon": row.mutated_codon,
        "aa": row.aa,
        "locale": row.locale,
...
def outputs_to_dataframe(
    outputs: Iterable[PegRNAOutput],
    target_tag: str = "",
) -> pd.DataFrame:
    rows = [_row_to_dict(o, target_tag=target_tag) for o in outputs]
    if not rows:
        return pd.DataFrame(columns=_PEGRNA_OUTPUT_COLS)
    df = pd.DataFrame(rows)
    return df[_PEGRNA_OUTPUT_COLS]


def build_dataframe_for_variants(
    ...
    all_rows: list[PegRNAOutput] = []
    for v in variants:
        all_rows.extend(build_outputs_for_variant(
            v,
            pbs_len=pbs_len,
            max_rtt_len=max_rtt_len,
            require_variant_in_rtt=require_variant_in_rtt,
    ...
    return outputs_to_dataframe(all_rows, target_tag=target_tag)