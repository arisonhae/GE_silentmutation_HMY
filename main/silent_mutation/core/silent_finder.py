...
def find_silent_candidates_for_variant(
    variant: Variant,
    pam_candidates: list[PamCandidate],
    codon_table: Optional[dict[str, str]] = None,
) -> dict[int, list[SilentCandidate]]:
    """
    For every PAM candidate of the given variant, compute its silent
    candidates. Return is keyed by id(pam).
    """
    if codon_table is None:
        codon_table = load_codon_table()

    result: dict[int, list[SilentCandidate]] = {}
    for pam in pam_candidates:
        result[id(pam)] = find_silent_candidates(
            variant=variant,
            pam=pam,
