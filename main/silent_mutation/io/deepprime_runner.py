"""
silent_mutation.io.deepprime_runner  (genet 0.17 version)

Option A — DeepPrime-first ranking using the *genet* package (genet.predict.DeepPrime).

Why genet instead of the local DeepPrime-main repo
--------------------------------------------------
We verified the local DeepPrime-main clone ships PE2max model weights that are
byte-identical to genet (20/20 .pt) and identical mean/std normalisation, yet it
scores ~2.6x lower than genet/SynDesign for the SAME pegRNA (e.g. canonical
PBS14/RTT33: DeepPrime-main 6.2 vs genet/reference 16.3). The divergence is in
DeepPrime-main's feature extraction (src/biofeat), so its scores do not match the
published SynDesign reference (deepcrispr.info). genet 0.17 reproduces the
reference, so scoring is done with genet here.

Interface preserved
-------------------
run_deepprime_silent(...) returns the SAME list-of-dict structure as before, so
server.py / index.html are unchanged. build_pam_from_wt74 is reused VERBATIM:
genet's 'Target' column is byte-identical to DeepPrime-main's 'WT74_On', and the
nick/RTT recovery (nick = Target_pos + 21, +strand) reproduces genet's Edit_pos
exactly (verified: Target@38 -> nick 59 -> edit@60 = Edit_pos 2).

genet lives in the lab 'genet' conda env. It is imported lazily; failures (and
...
def build_pam_from_wt74(seq_wt: str, wt74: str, rtlen: int, pbs_len: int) -> Optional[PamCandidate]:
    """Recover nick + strand from the WT 74-mer (genet's 'Target' column, which is
    byte-identical to DeepPrime-main's 'WT74_On') and build a PamCandidate whose
    ...
    Edit_pos exactly. Unchanged from the DeepPrime-main version.
    """
    L = len(seq_wt)
...
def run_deepprime_silent(
    ...
    """Run Option A with genet scoring. Returns an efficiency-ranked list of pegRNA
    dicts; each carries its full list of silent candidates (PegRNAOutput) under
    'outputs'. SAME return structure as the DeepPrime-main version, so server.py
    and index.html are unchanged.
